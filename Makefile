...
TAG = latest
IMAGE_NAME = ovenube
PROYECT_NAME = ovenube_peru
CONTAINER_NAME = ovenube
...
tag.all: ## Tag the docker containers, use me with: make tag.all version=1.0
	docker tag ${CONTAINER_NAME}-worker ${CONTAINER_OWNER}/${CONTAINER_NAME}-worker:${version}
	docker tag ${CONTAINER_NAME}-assets ${CONTAINER_OWNER}/${CONTAINER_NAME}-nginx:${version}

######## Push images
push.all: ## Push all the docker containers, use me with: make push.all version=1.0
	docker --config ~/.seethersan push ${CONTAINER_OWNER}/${CONTAINER_NAME}-worker:${version}
	docker --config ~/.seethersan push ${CONTAINER_OWNER}/${CONTAINER_NAME}-nginx:${version}

######## Pull images
pull.all: # Pull all the docker containers, use me with: make pull.all version=1.0
	docker --config ~/.seethersan pull ${CONTAINER_OWNER}/${CONTAINER_NAME}-worker:${version}
	docker --config ~/.seethersan pull ${CONTAINER_OWNER}/${CONTAINER_NAME}-nginx:${version}
